...
namespace GRaff
{
	...
	{
		private static Random _rnd = new Random(Time.GameTime);
		...
		public static int Integer() => _rnd.Next();
		...
		public static int Integer(int maxValue) => _rnd.Next(maxValue);
		...
		public static int Integer(int minValue, int maxValue) => _rnd.Next(minValue, maxValue);
		...
		{
			if (dice < 0) throw new ArgumentOutOfRangeException(nameof(dice), "Must be greater than or equal to 0");
			if (sides < 1) throw new ArgumentOutOfRangeException(nameof(sides), "Must be greater than 0");
